...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    TRAINER_CLASS(BATTLE_GIRL, "BATTLE GIRL", 10, ITEM_LUXURY_BALL),
...
u8 GetPoolIndex(u8 poolSize, u8 currNumMons, u16 *chosenSpecies, const struct Trainer *trainer)
{
    u16 foundSpecies;
    u8 rnd;
    u8 index;
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags)
{
						    ...
						    fixedLVL = GetMonData(&gPlayerParty[0], MON_DATA_LEVEL);
	{
        ...
        if (trainer->partySize != trainer->poolSize)
            usePool = TRUE;
        ...
        else
        {
            ...
            for (i = 0; i < monsCount; i++)
            {
				...
				min=1;
			    reallevel = min + rand;
                CreateMon(&party[i], poolData[poolIndex].species, reallevel, 0, TRUE, personalityValue, otIdType, fixedOtId);
