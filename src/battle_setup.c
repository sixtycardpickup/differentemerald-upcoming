...
u8 BattleSetup_GetTerrainId(void)
{
    ...
    if (MetatileBehavior_IsMudOrMudPuddle(tileBehavior))
        return BATTLE_TERRAIN_MUD;
...
void BattleSetup_StartTrainerBattle(void)
{
    ...
    if (FlagGet(FLAG_RANDOM_MON_DYNAMIC_LEVELS))
        gBattleTypeFlags = (BATTLE_TYPE_DYNAMIC_LEVELS | BATTLE_TYPE_RANDOM_MONS | BATTLE_TYPE_TRAINER);
    ...
    else if (FlagGet(FLAG_RANDOM_MON_DYNAMIC_LEVELS) && gNoOfApproachingTrainers == 2)
        gBattleTypeFlags = (BATTLE_TYPE_DOUBLE | BATTLE_TYPE_TWO_OPPONENTS | BATTLE_TYPE_DYNAMIC_LEVELS | BATTLE_TYPE_RANDOM_MONS | BATTLE_TYPE_TRAINER);
    else
